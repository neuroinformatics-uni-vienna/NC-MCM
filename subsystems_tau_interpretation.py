...
def _build_tau_network():
    return tf.keras.Sequential([
        layers.Flatten(),
        layers.Dense(50, activation='relu', kernel_regularizer=tf.keras.regularizers.l1(5e-5)),
...
wts = model.tau_m.get_weights()[0]
wts = wts.reshape((Xm_.shape[-2], Xm_.shape[-1], -1))
for i in range(wts.shape[1]):
    plt.figure()
    plt.imshow(wts[:,i], cmap='seismic', vmax=np.abs(wts).max(), vmin=-np.abs(wts).max())
    plt.show()
