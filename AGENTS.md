...
| 2026-05-21 | `3e217e0` | Prompt 037: lifecycle segmentation. `ncmcm/data_loaders/bandit_task.py`: new `segment_policy=None` parameter + `_create_lifecycle_trial_indices()` (windows = [trial.start, next_trial.start-1], last trial → last_timestamp_ms); overrides trial_indices in `load_data()` when `segment_policy='lifecycle_start_to_next_start'`; `segment_policy` added to cache key, `_save_to_cache()`, `_load_from_cache()` validation. `scripts/bandit_gridsearch.py`: `--segment_policy` CLI arg (grid-searchable). `ncmcm/experiment_archive/manifest.py`: `segment_policy` in model section. `scripts/verify_lifecycle_segmentation.py` (new): 4-criterion smoke test. Results: lifecycle coverage 100% (vs 57% default), reward=3,869/no-reward=9,149 visible in train, proportions match full-session, no regression in default config. |
