...
_FIGURE_CAPTIONS = {
    ...
    "33_session_timeseries_predictions.html": (
        "Interactive session-wide prediction timeline. The main continuous trace shows the model's predicted probability "
        "of choosing the right arm, rescaled to [-1 (strong left), +1 (strong right)]. Per-window predictions are plotted "
        "as the continuous line; per-trial markers at choice time show the true trial choice and the model's predicted class. "
        "Color semantics follow the same rules as the per-trial markers: by default colors encode prediction correctness "
        "(green = prediction matches the true choice, red = prediction incorrect). An alternative 'color by train/test' mode "
        "repaints traces and markers to indicate whether a window came from the training set or a held-out test trial. "
        "Vertical dotted lines mark reward-probability block transitions and are annotated 'Better L' / 'Better R'.<br><br>"
        "<strong>Interactive controls:</strong> use the subset selector (All / Train only / Test only) to filter windows, "
        "predicted markers, and true markers; use the color-mode toggle to switch between correctness and train/test coloring. "
        "These filters are implemented client-side using exported boolean masks that map trace names to plot indices, so the "
        "controls remain robust when traces are inserted or reordered.<br><br>"
        "<strong>Wrong-prop curve (right axis):</strong> this auxiliary trace visualises a locally averaged estimate of the "
        "fraction of incorrect predictions over time (y-range 0–1). It is computed in five clear steps: "
        "(1) each sliding-window scalar prediction p is thresholded to a predicted class (class = 1 if p >= 0, else 0); "
        "(2) each window is associated with its parent trial's true choice label (0/1); "
        "(3) build a boolean array `correct_win` where True means predicted class == true label, then `win_wrong = (~correct_win).astype(float)` "
        "produces 1.0 for incorrect windows and 0.0 for correct windows; "
        "(4) compute a short running average across neighbouring windows by convolving `win_wrong` with a uniform kernel of length K "
        "(we choose an adaptive K = max(5, round(5% of the total number of windows)) and use `mode='same'`), yielding `wrong_prop_raw`; "
        "(5) for display we apply a gentle Gaussian smoothing: `wrong_prop = gaussian_filter1d(wrong_prop_raw, sigma=max(1.0, 0.15 * K))`. "
        "The plotted curve therefore reflects a smoothed estimate of the local proportion of wrong predictions rather than an exact per-trial error count. "
        "Convolution with `mode='same'` and subsequent Gaussian smoothing introduce mild edge effects and a small temporal smoothing/lag; "
        "interpret values as local trends (0 = mostly correct locally; 1 = mostly incorrect locally). Hover to see the smoothed value at any time. "
        "To change smoothing aggressiveness, adjust the K / sigma logic in this generator or request an interactive smoothing slider."
    ),
}
